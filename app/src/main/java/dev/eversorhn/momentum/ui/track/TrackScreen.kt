package dev.eversorhn.momentum.ui.track
...
import dev.eversorhn.momentum.domain.fidelity.FidelityReplay
import dev.eversorhn.momentum.ui.debrief.DebriefContent
import dev.eversorhn.momentum.ui.forecast.formatDuration
import dev.eversorhn.momentum.ui.forecast.formatPace
...
fun TrackScreen(duel: Boolean, onDone: () -> Unit) {
    ...
    uiState.recoverable?.let { r ->
        dev.eversorhn.momentum.ui.theme.CorpoDialog(
            ...
            body = when (r.mode) {
                TrackMode.OUTDOOR -> "MOMENTUM was stopped mid-session. Captured so far: ${"%.2f".format(r.distanceMeters / 1000.0)} km over ${formatElapsed(r.movingSeconds)} moving. Save it?"
                TrackMode.INDOOR -> "MOMENTUM was stopped mid-session. ${formatElapsed(r.durationSeconds)} were timed. Save it? You'll be asked for the distance next."
            },
    ...
    val activeActivity = dev.eversorhn.momentum.domain.activity.Activities.byKey(
        (LocalContext.current.applicationContext as dev.eversorhn.momentum.MomentumApplication).repository.activeActivityType
    )
    ...
    Column(
        ...
        when {
            ...
            !uiState.starting && !snapshot.isTracking && !(uiState.mode == TrackMode.OUTDOOR && !hasLocationPermission) -> {
                ...
                val act = dev.eversorhn.momentum.domain.activity.Activities.byKey(
                    (LocalContext.current.applicationContext as dev.eversorhn.momentum.MomentumApplication).repository.activeActivityType
                )
                ...
                if (opponent != null && opponent.forecastPaceSecPerKm != null) {
                    CorpoPanel {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            StatTile(if (act.usesSpeed) "Speed" else "Pace", dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed((if (isDuel) opponent.duelTargetPaceSecPerKm else null) ?: opponent.forecastPaceSecPerKm, act.key), accent = if (opponent.isHorde) Alert else Cyan)
            ...
            uiState.mode == TrackMode.OUTDOOR && !hasLocationPermission -> {
                ScreenTitle(screenEyebrow, "MOMENTUM needs your location")
                Text(
            ...
            snapshot.isTracking -> {
                ...
                if (opponent?.forecastPaceSecPerKm != null) {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        StatTile(
                            "$opponentName ${dev.eversorhn.momentum.domain.activity.Activities.paceWord(activeActivity.key).lowercase()}",
                            dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(opponent.forecastPaceSecPerKm, activeActivity.key),
                            accent = Cyan,
                        )
...
private fun DuelBriefing(opponent: LiveOpponent?, activityKey: String?) {
    CorpoPanel(tone = PanelTone.WARN) {
        ...
        Text(
            opponent?.duelTargetPaceSecPerKm?.let { "Beat ${dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(it, activityKey)} — its strongest session" }
...
private fun LiveSession(
    snapshot: dev.eversorhn.momentum.tracking.TrackingSnapshot,
    mode: TrackMode,
    ...
    val activity = dev.eversorhn.momentum.domain.activity.Activities.byKey(
        (LocalContext.current.applicationContext as dev.eversorhn.momentum.MomentumApplication).repository.activeActivityType
    )
    // Motor-assisted / wheeled activities read better in km/h than min/km.
    val showSpeed = activity.usesSpeed
    fun paceOrSpeed(secPerKm: Double): String = dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(secPerKm, activity.key)
    ...
    if (mode == TrackMode.OUTDOOR && projection != null && opponent != null) {
        ...
        CorpoPanel(tone = when {
            ...
            if (opponent.isHorde) {
                ...
                dev.eversorhn.momentum.ui.theme.ProximityRadar(separationMeters = p.separationMeters ?: 600)
                ...
                Row(verticalAlignment = Alignment.Bottom, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    dev.eversorhn.momentum.ui.theme.LiveNumber(
                ...
                referenceDistance?.takeIf { it > 0 }?.let { target ->
                    ...
                    dev.eversorhn.momentum.ui.theme.RaceBar(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    StatTile(
                        ...
                        neededPace?.let { dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(it, activity.key) } ?: (p.projectedFinishSeconds?.let { formatDuration(it) } ?: "—"),
    ...
    if (mode == TrackMode.OUTDOOR) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (activity.key == "HIKING" || activity.key == "CYCLING" || activity.key == "E_BIKE" || activity.key == "HAND_CYCLE" || snapshot.elevationGainMeters >= 20) {
                ...
                StatTile(
                    "Climb",
                    "${climb.toInt()} m",
                    sub = if (snapshot.barometricClimbMeters != null) "barometer" else snapshot.splitSeconds.takeIf { it.size >= 2 }?.let { sp -> dev.eversorhn.momentum.domain.route.RouteMetrics.consistency(sp)?.let { "steady ${(it * 100).toInt()}%" } },
                )
            }
        }
        snapshot.heartRate?.let { bpm ->
            ...
            if (range != null) {
                dev.eversorhn.momentum.ui.theme.EffortBar(bpm = bpm, lowBpm = range.first, highBpm = range.second)
            }
        }
        ...
        if (pace != null && referencePace != null && opponent != null) {
            ...
            val steadyNow = snapshot.splitSeconds.takeIf { it.size >= 2 }?.let { dev.eversorhn.momentum.domain.route.RouteMetrics.consistency(it) }
        ...
        projection?.splits?.takeIf { it.isNotEmpty() }?.let { splits ->
            CorpoPanel {
                ...
                dev.eversorhn.momentum.ui.theme.SplitBars(
                ...
                callouts.asReversed().take(3).forEach { c ->
                    Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.Top) {
                        ...
                        Text(
                            ...
                            style = MaterialTheme.typography.bodyMedium,
                            color = when (c.zone) {
                                dev.eversorhn.momentum.domain.live.LiveZone.BEHIND -> Alert
                                dev.eversorhn.momentum.domain.live.LiveZone.AHEAD -> MaterialTheme.colorScheme.onSurfaceVariant
                                dev.eversorhn.momentum.domain.live.LiveZone.LEVEL -> MaterialTheme.colorScheme.onSurface
                            },
                        )
                    }
                }
...
private fun divergenceLabel(
    ...
    activity: dev.eversorhn.momentum.domain.activity.Activity,
): String {
