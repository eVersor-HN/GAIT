package dev.eversorhn.momentum.ui.settings
...
import dev.eversorhn.momentum.ui.theme.CorpoChip
...
fun SettingsScreen(onDone: () -> Unit, onWiped: () -> Unit) {
    val viewModel: SettingsViewModel = momentumViewModel()
    ...
    if (confirmWipe) {
        dev.eversorhn.momentum.ui.theme.CorpoDialog(
            ...
            confirmKind = dev.eversorhn.momentum.ui.theme.ButtonKind.RISK,
        )
    }
    ...
    Column(
        ...
        if (state.isHorde) {
            dev.eversorhn.momentum.ui.theme.CollapsiblePanel(title = "Intensity", summary = HordeIntensity.label(state.hordeIntensity)) {
                dev.eversorhn.momentum.ui.theme.Segmented(
            ...
            dev.eversorhn.momentum.ui.theme.CollapsiblePanel(title = "Name", summary = state.twinName) {
                OutlinedTextField(
        ...
        val appRepo = (androidx.compose.ui.platform.LocalContext.current.applicationContext as dev.eversorhn.momentum.MomentumApplication).repository
        ...
        run {
            val monitor = androidx.compose.runtime.remember { dev.eversorhn.momentum.sensors.HeartRateMonitor(appCtx) }
            var paired by androidx.compose.runtime.remember {
                androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.sensors.HeartRatePrefs.name(appCtx))
            }
            var scanning by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(false) }
            val found = androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateListOf<dev.eversorhn.momentum.sensors.HeartRateMonitor.Found>() }
                ...
                dev.eversorhn.momentum.ui.theme.CollapsiblePanel(
                    ...
                    if (paired != null) {
                        CorpoButton("Forget monitor", onClick = {
                            dev.eversorhn.momentum.sensors.HeartRatePrefs.forget(appCtx); paired = null
                        ...
                        found.forEach { device ->
                            dev.eversorhn.momentum.ui.enrol.SelectRow(
                                title = device.name,
                                detail = device.address,
                                selected = false,
                                onClick = {
                                    dev.eversorhn.momentum.sensors.HeartRatePrefs.remember(appCtx, device.address, device.name)
                                    paired = device.name
                                },
                            )
                        }
        ...
        if (dev.eversorhn.momentum.health.HealthImport.isAvailable(appCtx)) {
            ...
            val hcPermLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
                ...
                if (granted.containsAll(dev.eversorhn.momentum.health.HealthImport.PERMISSIONS)) {
                    scope.launch {
                        val r = dev.eversorhn.momentum.health.HealthImport.importRecent(appCtx, appRepo)
            ...
            dev.eversorhn.momentum.ui.theme.CollapsiblePanel(title = "Health Connect", summary = "Import last 30 days of ${dev.eversorhn.momentum.domain.activity.Activities.byKey(appRepo.activeActivityType).label.lowercase()} from other apps") {
                Text(
                    "Reads exercise sessions and distance from Health Connect (watch, other tracker apps) and adds the ones MOMENTUM doesn't have — as baseline material, tagged HEALTH.",
                    style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
                CorpoButton("Import from Health Connect", onClick = {
                    scope.launch {
                        if (dev.eversorhn.momentum.health.HealthImport.hasPermissions(appCtx)) {
                            val r = dev.eversorhn.momentum.health.HealthImport.importRecent(appCtx, appRepo)
                            hcNote = r.error?.let { "Import failed: $it" } ?: "${r.imported} imported · ${r.skipped} skipped"
                        } else hcPermLauncher.launch(dev.eversorhn.momentum.health.HealthImport.PERMISSIONS)
                    }
                ...
                var exportOn by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.health.ExportPrefs.isEnabled(appCtx)) }
                val exportLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
                    androidx.health.connect.client.PermissionController.createRequestPermissionResultContract()
                ) { granted ->
                    val ok = granted.containsAll(dev.eversorhn.momentum.health.HealthExport.PERMISSIONS)
                    dev.eversorhn.momentum.health.ExportPrefs.setEnabled(appCtx, ok)
                    exportOn = ok
                }
                dev.eversorhn.momentum.ui.theme.CorpoSwitch(
                    label = "Write sessions back",
                    description = "Every session you finish in MOMENTUM is written to Health Connect, so your rings, your watch and your other apps see the same run. Sessions imported from there are never written back.",
                    checked = exportOn,
                    onChange = { want ->
                        if (want) exportLauncher.launch(dev.eversorhn.momentum.health.HealthExport.PERMISSIONS)
                        else { dev.eversorhn.momentum.health.ExportPrefs.setEnabled(appCtx, false); exportOn = false }
                    },
                )
            }
        }

        dev.eversorhn.momentum.ui.theme.CollapsiblePanel(title = "Asset transfer", summary = "Export the division's file on you · import someone else's asset") {
        Text(
            "Export the division's assessment of you as a text block. Another MOMENTUM can import it: your asset then lives on in their division — climbs, gets reviewed, can be culled. Import someone's block below to take their asset into yours.",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        ...
        var importedList by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf<List<dev.eversorhn.momentum.data.db.entity.ImportedAssetEntity>>(emptyList()) }
        ...
        CorpoButton("Export my asset · share", onClick = {
            scope.launch {
                ...
                val asset = dev.eversorhn.momentum.domain.transfer.AssetTransfer.assess(
                    sessions, dev.eversorhn.momentum.domain.ledger.Ledger.from(sessions), (profile.fidelity * 100).toInt(),
                    exportName.ifBlank { "Asset vs. ${profile.twinName}" },
                    dev.eversorhn.momentum.domain.roster.AssetKind.HUMAN_M, enrolled, today, profile.restDayMask, zone,
                )
                val text = dev.eversorhn.momentum.domain.transfer.AssetTransfer.encode(asset)
                val send = android.content.Intent(android.content.Intent.ACTION_SEND).apply {
                    type = "text/plain"
                    putExtra(android.content.Intent.EXTRA_SUBJECT, "MOMENTUM asset transfer · ${asset.id}")
        ...
        OutlinedTextField(
            ...
            label = { Text("Paste a MOMENTUM-ASSET block") },
            minLines = 3,
            maxLines = 6,
            modifier = Modifier.fillMaxWidth(),
        )
        CorpoButton("Import asset", onClick = {
            val parsed = dev.eversorhn.momentum.domain.transfer.AssetTransfer.decode(importText)
            if (parsed == null) { importNote = "That isn't a MOMENTUM-ASSET block."; return@CorpoButton }
            scope.launch {
        ...
        var voiceOn by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.audio.VoicePrefs.isEnabled(appCtx)) }
        CorpoPanel {
            SectionLabel("Sound & notifications")
            var hapticsOn by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.audio.HapticPrefs.isEnabled(appCtx)) }
            dev.eversorhn.momentum.ui.theme.CorpoSwitch(
                ...
                checked = hapticsOn,
                onChange = { dev.eversorhn.momentum.audio.HapticPrefs.setEnabled(appCtx, it); hapticsOn = it },
            )
            var soundOn by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.audio.SoundPrefs.isEnabled(appCtx)) }
            dev.eversorhn.momentum.ui.theme.CorpoSwitch(
                label = "Session sound",
                ...
                checked = soundOn,
                onChange = { dev.eversorhn.momentum.audio.SoundPrefs.setEnabled(appCtx, it); soundOn = it },
            )
            dev.eversorhn.momentum.ui.theme.CorpoSwitch(
                label = "Spoken readout",
                description = "Reads the live figures aloud at kilometre marks and lead changes. Ducks your music.",
                checked = voiceOn,
                onChange = { dev.eversorhn.momentum.audio.VoicePrefs.setEnabled(appCtx, it); voiceOn = it },
            )

        // --- Notifications: the exit dialog can mute them; this is where they come back ---
        val ctx = androidx.compose.ui.platform.LocalContext.current
        var muted by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.notification.NotificationPrefs.isMuted(ctx)) }
            dev.eversorhn.momentum.ui.theme.CorpoSwitch(
                label = "Notifications",
                ...
                checked = !muted,
                onChange = { dev.eversorhn.momentum.notification.NotificationPrefs.setMuted(ctx, !it); muted = !it },
            )
        }

        if (dev.eversorhn.momentum.BuildConfig.DEBUG) {
                ...
                CorpoButton("Seed 6 sessions", onClick = {
                    scope.launch {
                        ...
                        repeat(6) { i ->
                            ...
                            appRepo.logSession(
                                dev.eversorhn.momentum.data.db.entity.SessionEntity(
                                    ...
                                    dataSource = dev.eversorhn.momentum.data.db.entity.SessionSource.MANUAL,
                                    stake = 1,
                                )
                            )
                        }
        ...
        var demoLoaded by androidx.compose.runtime.remember {
            androidx.compose.runtime.mutableStateOf(dev.eversorhn.momentum.domain.demo.DemoSeeder.isLoaded(appCtx, appRepo.activeProfileId))
        }
        dev.eversorhn.momentum.ui.theme.CollapsiblePanel(
            title = "Demo data",
            ...
            if (demoLoaded) {
                CorpoButton("Remove demo data", onClick = {
                    scope.launch {
                        val n = dev.eversorhn.momentum.domain.demo.DemoSeeder.remove(appRepo, appCtx)
                        viewModel.refresh()
                        demoLoaded = false
                ...
                CorpoButton("Load demo data", onClick = {
                    scope.launch {
                        dev.eversorhn.momentum.domain.demo.DemoSeeder.seed(appRepo, appCtx)
                        viewModel.refresh()
                        demoLoaded = true
        ...
        dev.eversorhn.momentum.ui.theme.CollapsiblePanel(
            title = "About",
            summary = "MOMENTUM ${dev.eversorhn.momentum.BuildConfig.VERSION_NAME} · Marco Aurelio Fattizzo",
        ) {
            Text(
                "MOMENTUM — local movement tracking with one opponent: a model of you, built from your " +
            ...
            FootNote("Version ${dev.eversorhn.momentum.BuildConfig.VERSION_NAME} (${dev.eversorhn.momentum.BuildConfig.VERSION_CODE})")
            CorpoButton(
                    ...
                    runCatching {
                        appCtx.startActivity(
                            android.content.Intent(
                                ...
                                android.net.Uri.parse("https://github.com/eVersor-HN/MOMENTUM"),
        ...
        dev.eversorhn.momentum.ui.theme.CollapsiblePanel(title = "Danger zone", summary = "Erase everything on this device", tone = dev.eversorhn.momentum.ui.theme.PanelTone.WARN) {
