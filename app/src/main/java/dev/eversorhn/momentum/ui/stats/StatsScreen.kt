...
fun StatsScreen(onDone: () -> Unit) {
    val viewModel: StatsViewModel = momentumViewModel()
    ...
    Column(
        ...
        CorpoPanel {
            ...
            Text(
                "Avg ${dev.eversorhn.momentum.domain.activity.Activities.paceWord((androidx.compose.ui.platform.LocalContext.current.applicationContext as dev.eversorhn.momentum.MomentumApplication).repository.activeActivityType).lowercase()} ${state.avgPaceLabel} · ${state.metricLabel} now ${state.metricPercent}%",
            ...
            dev.eversorhn.momentum.ui.theme.CollapsiblePanel(
                ...
                state.transmissions.forEach { (date, line) ->
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        Text(line, style = MaterialTheme.typography.bodyMedium, color = dev.eversorhn.momentum.ui.theme.TextDim, modifier = Modifier.weight(1f))
                    }
                }
            }
        }
        FootNote("Swipe back for Analysis and Forecast", color = dev.eversorhn.momentum.ui.theme.TextFaint)
    }
}
