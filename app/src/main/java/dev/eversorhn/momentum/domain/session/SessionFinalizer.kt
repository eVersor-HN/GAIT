package dev.eversorhn.momentum.domain.session
...
import dev.eversorhn.momentum.data.repository.ACTIVITY_RUNNING
import dev.eversorhn.momentum.data.repository.MomentumRepository
import dev.eversorhn.momentum.domain.composure.ComposureEngine
import dev.eversorhn.momentum.domain.composure.ComposureState
...
import dev.eversorhn.momentum.domain.wager.WagerPolicy
import dev.eversorhn.momentum.domain.restdays.RestDayPolicy
import dev.eversorhn.momentum.domain.trial.DecommissionTrial
import dev.eversorhn.momentum.notification.TwinNotifier
import dev.eversorhn.momentum.ui.forecast.formatDistanceKm
...
class SessionFinalizer(
    ...
    suspend fun finalize(
        ...
        val activity = dev.eversorhn.momentum.domain.activity.Activities.byKey(repository.activeActivityType)
        val routePoints = dev.eversorhn.momentum.domain.route.RouteMetrics.decode(route)
        val priorRoutes = priorSessions.mapNotNull { it.route }.take(60).map { dev.eversorhn.momentum.domain.route.RouteMetrics.decode(it) }
        val novelty = dev.eversorhn.momentum.domain.route.RouteMetrics.novelty(routePoints, priorRoutes)
        val consistency = dev.eversorhn.momentum.domain.route.RouteMetrics.consistency(splitSeconds)
        ...
        appContext?.let { dev.eversorhn.momentum.health.HealthExport.write(it, storedSession, indoor = dataSource != SessionSource.GPS) }
        ...
        return DebriefResult(
            ...
            forecastPaceLabel = forecast?.let { dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(it.forecastPaceSecPerKm, repository.activeActivityType) } ?: "—",
            actualPaceLabel = dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(avgPace, repository.activeActivityType),
            paceWord = dev.eversorhn.momentum.domain.activity.Activities.paceWord(repository.activeActivityType),
            avgHeartRate = avgHeartRate,
            maxHeartRate = maxHeartRate,
            marginLabel = forecast?.let { f ->
                val a = dev.eversorhn.momentum.domain.activity.Activities.byKey(repository.activeActivityType)
                if (a.usesSpeed) {
            ...
            duel = verdict?.let {
                DuelOutcome(
                    verdict = it,
                    targetPaceLabel = dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(duelTarget!!, repository.activeActivityType),
