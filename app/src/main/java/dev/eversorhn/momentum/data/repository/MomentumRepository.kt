package dev.eversorhn.momentum.data.repository
 ...
 */
class MomentumRepository(private val db: MomentumDatabase, private val appContext: Context? = null) {

    private val prefs get() = appContext?.getSharedPreferences("momentum_repository", Context.MODE_PRIVATE)
    ...
    suspend fun deleteProfile(profile: TwinProfileEntity) {
        ...
        appContext?.let { dev.eversorhn.momentum.domain.demo.DemoRecord.clear(it, profile.id) }
    ...
    suspend fun wipeAll() {
        ...
        appContext?.let { ctx -> profileIds.forEach { dev.eversorhn.momentum.domain.demo.DemoRecord.clear(ctx, it) } }
        activeProfileId = 0L
    }
}
