...
class LocationTrackingService : Service() {
    ...
    companion object {
        const val ACTION_START_OUTDOOR = "dev.eversorhn.momentum.tracking.START_OUTDOOR"
        const val ACTION_START_INDOOR = "dev.eversorhn.momentum.tracking.START_INDOOR"
        const val ACTION_STOP = "dev.eversorhn.momentum.tracking.STOP"
        ...
        /** 30 m/s ≈ 108 km/h: faster than any activity MOMENTUM tracks, so it can only be a bad fix. */
    ...
    private val routePoints = ArrayList<dev.eversorhn.momentum.domain.route.RouteMetrics.Point>()
    ...
    private fun startHeartRate() {
        val address = dev.eversorhn.momentum.sensors.HeartRatePrefs.address(applicationContext) ?: return
        heart.resetSummary()
    ...
    private fun liveNotification(mode: TrackingMode, s: TrackingSnapshot? = null): android.app.Notification {
        ...
        val activityKey = opp?.activityKey
            ?: (applicationContext as? dev.eversorhn.momentum.MomentumApplication)?.repository?.activeActivityType
        fun fmt(p: Double) = dev.eversorhn.momentum.domain.activity.Activities.formatPaceOrSpeed(p, activityKey)
        ...
        val detail: String?
        if (!live.isTracking) {
            title = if (mode == TrackingMode.OUTDOOR) "MOMENTUM · waiting for GPS" else "MOMENTUM · timing"
            ...
            title = "MOMENTUM · ${mmss(live.elapsedSeconds)}"
            text = opp?.let { o ->
            ...
            title = "MOMENTUM · ${mmss(live.movingSeconds)} · %.2f km".format(live.distanceMeters / 1000.0)
        ...
        return NotificationCompat.Builder(ctx, TwinNotifier.trackingChannelId(ctx))
            ...
            .setSmallIcon(dev.eversorhn.momentum.R.drawable.ic_notification)
    ...
    private val voice by lazy { dev.eversorhn.momentum.audio.SessionVoice(applicationContext) }
    /** The pocket channel: what the session feels like without looking at anything. */
    private val haptics by lazy { dev.eversorhn.momentum.audio.SessionHaptics(applicationContext) }
    /** The horde, heard behind you. */
    private val presence by lazy { dev.eversorhn.momentum.audio.HordePresence(applicationContext) }
    /** A paired strap or watch, if there is one. Pace says how fast; this says what it cost. */
    private val heart by lazy { dev.eversorhn.momentum.sensors.HeartRateMonitor(applicationContext) }
    /** The instruments the phone already has: barometer, step counter, compass. */
    private val sensors by lazy { dev.eversorhn.momentum.sensors.SessionSensors(applicationContext) }
        ...
        tickerJob = serviceScope.launch {
            ...
            while (true) {
                ...
                if (snap.isTracking && snap.mode == TrackingMode.OUTDOOR) {
                    ...
                    dev.eversorhn.momentum.wear.WearPublisher.publishLive(applicationContext, figures, snap.heartRate)
                }
    ...
    private fun onNewLocation(location: Location) {
        ...
        TrackingSessionState.update { s ->
            ...
            if (moved || previous == null) {
                val p = dev.eversorhn.momentum.domain.route.RouteMetrics.Point(location.latitude, location.longitude)
                if (dev.eversorhn.momentum.domain.route.RouteMetrics.shouldKeep(routePoints.lastOrNull(), p)) routePoints += p
                if (location.hasAltitude()) {
                    val alt = location.altitude
                    val la = lastAltitude
                    if (la == null) lastAltitude = alt
                    else if (kotlin.math.abs(alt - la) >= dev.eversorhn.momentum.domain.route.RouteMetrics.CLIMB_STEP_METERS) { if (alt > la) elevationGain += alt - la; lastAltitude = alt }
                }
            }
            ...
            s.copy(
                ...
                routePolyline = if (routePoints.size % 4 == 0 || s.routePolyline.isEmpty()) dev.eversorhn.momentum.domain.route.RouteMetrics.encode(routePoints) else s.routePolyline,
