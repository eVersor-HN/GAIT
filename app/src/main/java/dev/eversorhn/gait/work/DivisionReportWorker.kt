...
import dev.eversorhn.gait.domain.opponent.OpponentActivity
...
class DivisionReportWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        val app = applicationContext as? GaitApplication ?: return Result.success()
        ...
        for (profile in profiles) {
            ...
            val activityLine = if (!activity.active) null else {
                ...
                val pace = activity.paceSecPerKm?.let {
                    dev.eversorhn.gait.domain.activity.Activities.formatPaceOrSpeed(it, profile.activityType)
                }
    ...
    companion object {
        const val UNIQUE_WORK_NAME = "gait-division-report"
    }
}
