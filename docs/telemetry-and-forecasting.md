...
- **Declared rest days.** Users mark specific weekdays (e.g. "never Sundays") as rest days. On a declared rest day a session can still be logged and still counts as history, but Fidelity is frozen rather than moved, Composure stays neutral, and no notification fires. *(Built and enforced in `SessionFinalizer` since v0.4.0 — earlier versions only displayed a label. The same gating applies during vacation, and `IdleTauntWorker` stays silent during vacation.)*
