...
Originally a screen inside the main MOMENTUM app; moved out into its own Gradle module (`:simdemo`, applicationId `dev.eversorhn.momentum.simdemo`) so the real app stays free of demo/dummy content.
...
- **It's a better artifact for its actual job.** A tiny, standalone, shareable APK is what you hand someone to show them what MOMENTUM feels like — a marketing/preview teaser — without asking them to install the real app, grant it location permission, or go through Setup first.
...
A persistent red **"DEMO — NOT A REAL SESSION"** badge stays visible throughout, on top of the same cyberpunk-corpo visual chrome as the main app (duplicated, not shared — see below) so it's immediately recognizable as MOMENTUM without being mistakable for the real thing.
